...
import { InvoiceContext, invoiceReducer, initialState } from './InvoiceContextStore';

export function InvoiceProvider({ children }: { children: React.ReactNode }) {
  const [state, dispatch] = useReducer(invoiceReducer, initialState);

  const DATA_VERSION = 'v2-uk';

  useEffect(() => {
    const savedVersion = localStorage.getItem('invoices_version');
    const saved = localStorage.getItem('invoices');
    if (saved && savedVersion === DATA_VERSION) {
      dispatch({ type: 'SET_INVOICES', payload: JSON.parse(saved) });
    } else {
      // Clear stale data and load fresh seed
      ...
      const mockData: Invoice[] = [
        ...
        },
        {
          ...
          status: 'pending',
          senderAddress: { street: '19 Union Terrace', city: 'London', postCode: 'E1 3EZ', country: 'United Kingdom' },
          clientAddress: { street: '3 Big Daddy Boulevard', city: 'Bradford', postCode: 'BD1 9PB', country: 'United Kingdom' },
          items: [
            { id: '1', name: 'Web Design', quantity: 1, price: 6155.91, total: 6155.91 },
          ],
          total: 6155.91,
        },
        {
          ...
          clientEmail: 'anita.wainwright@example.com',
          status: 'draft',
          senderAddress: { street: '19 Union Terrace', city: 'London', postCode: 'E1 3EZ', country: 'United Kingdom' },
      ...
      dispatch({ type: 'SET_INVOICES', payload: mockData });
    }
  }, []);
  ...
  useEffect(() => {
    if (state.invoices.length > 0) {
      ...
      localStorage.setItem('invoices_version', DATA_VERSION);
    }
  }, [state.invoices]);
